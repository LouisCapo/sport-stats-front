...
constructor(@Inject(ENV_TOKEN) private _environments,
  ...
  getMatchesList(sportTypeCode: Number, isComplited: Boolean): Observable<IMatchesListInterface[] | IErrorRequest> {
    const params = new HttpParams().set('sportTypeCode', sportTypeCode.toString()).set('isCompleted', isComplited.toString());
    return this._http.get<IMatchesListInterface[] | IErrorRequest>(`${this._environments.apiUrl}/api/games/games-list`, { params });
  }
